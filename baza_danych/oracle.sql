...
CREATE SEQUENCE klienci_seq MINVALUE 1 MAXVALUE 999999999 START WITH 1 INCREMENT BY 1 NOCACHE;
...
CREATE SEQUENCE umowy_seq MINVALUE 1 MAXVALUE 999999999 START WITH 1 INCREMENT BY 1 NOCACHE;
CREATE TABLE UMOWY (
...
CREATE SEQUENCE wplaty_seq MINVALUE 1 MAXVALUE 999999999 START WITH 1 INCREMENT BY 1 NOCACHE;
CREATE TABLE WPLATY (
...
CREATE SEQUENCE odsetki_seq MINVALUE 1 MAXVALUE 999999999 START WITH 1 INCREMENT BY 1 NOCACHE;
...
CREATE OR REPLACE PROCEDURE loguj_start(p_nazwa_procesu in VARCHAR) IS BEGIN
INSERT INTO status_wykonania (nazwa_procesu, data_startu, data_konca)
VALUES (p_nazwa_procesu, CURRENT_TIMESTAMP, null);
COMMIT;
END;
CREATE OR REPLACE PROCEDURE loguj_stop(p_nazwa_procesu in VARCHAR) IS BEGIN
UPDATE status_wykonania
SET data_konca = CURRENT_TIMESTAMP
WHERE nazwa_procesu = p_nazwa_procesu;
COMMIT;
END;
CREATE PROCEDURE insert_tmp_data_danych_rok (data_danych IN DATE) IS BEGIN
...
--END;
CREATE OR REPLACE PROCEDURE laduj_dane_kursy_walut (
        p_data_danych IN DATE,
        p_data_danych_ostatnia IN DATE
    ) IS BEGIN
...
--END;
CREATE OR REPLACE PROCEDURE laduj_umowy_pln IS BEGIN
...
END;
CREATE PROCEDURE laduj_wplaty_pln IS BEGIN
...
from wplaty w
    ...
    and w.waluta = k.kod;
END;
CREATE PROCEDURE laduj_suma_wplat_pln IS BEGIN
...
END;
CREATE OR REPLACE PROCEDURE laduj_rozliczenie_wplat is BEGIN
DECLARE v_RESZTA NUMBER;
v_KWOTA NUMBER;
V_SUMA_WPLAT NUMBER;
BEGIN FOR k IN (
    SELECT u.id_umowy,
...
IF V_SUMA_WPLAT >= k.KWOTA_PLN THEN V_RESZTA := V_SUMA_WPLAT - k.KWOTA_PLN;
V_KWOTA := 0;
ELSIF V_SUMA_WPLAT < 0 THEN V_RESZTA := V_SUMA_WPLAT - k.KWOTA_PLN;
V_KWOTA := k.KWOTA_PLN;
ELSE V_RESZTA := V_SUMA_WPLAT - k.KWOTA_PLN;
...
END;
END;
CREATE OR REPLACE PROCEDURE laduj_odsetki IS BEGIN
...
END;
CREATE PROCEDURE laduj_zalegle_umowy IS BEGIN
...
END;
CREATE PROCEDURE laduj_saldo_klienta IS BEGIN
...
WHERE k.status = 'Active';
END;
CREATE OR REPLACE PROCEDURE czysc_tabele is BEGIN execute immediate 'TRUNCATE TABLE ODSETKI';
